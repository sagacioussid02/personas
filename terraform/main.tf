...
resource "aws_apigatewayv2_route" "post_chat_debate" {
  api_id    = aws_apigatewayv2_api.main.id
  route_key = "POST /chat/debate"
  target    = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "post_debate_turn" {
  api_id    = aws_apigatewayv2_api.main.id
  route_key = "POST /debate/turn"
  target    = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}
...
resource "aws_apigatewayv2_route" "get_public_personas" {
  api_id    = aws_apigatewayv2_api.main.id
  route_key = "GET /public-personas"
  target    = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}
